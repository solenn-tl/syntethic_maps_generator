...
try:
    ...
    cur.execute("SELECT id FROM lieuditnonhabite;")  # Assuming 'id' is the primary key
    ids = [row[0] for row in cur.fetchall()]
    
    # Use the ids to update each row's rotation value
    for i, rotation in enumerate(rotation_values):
        cur.execute("""
            UPDATE lieuditnonhabite
            SET rotation = %s
            WHERE id = %s;
        """, (rotation, ids[i]))
